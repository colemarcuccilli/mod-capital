...
// Interface for form data (Mostly same as Gap)
interface PMLFormData {
  firstName: string;
  lastName: string;
  email: string;
  phone: string;
  ...
  dealLength: string;
  lienPosition: string; // Options: 1st, 2nd, Other
  hasCollateral: string; 
  fundsUsage: string; 
  additionalDetails?: string;
  dealDocuments?: File | null;
}

const PMLForm: React.FC = () => {
  const navigate = useNavigate();
  const [formData, setFormData] = useState<PMLFormData>({
    firstName: '',
    lastName: '',
    email: '',
    phone: '',
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    console.log("PML Form Data Submitted:", formData); // Updated log message
    setTimeout(() => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-background p-8 rounded-lg shadow-lg max-w-2xl mx-auto">
      <h3 className="text-2xl font-semibold text-primary mb-6 text-center">Private Money Loan Request</h3> {/* Updated Title */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
         <div>
          ...
          <div>
            ...
            <input type="number" name="amountNeeded" id="amountNeeded" required value={formData.amountNeeded} onChange={handleChange} className="input-field" placeholder="e.g., 250000"/>
          </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         <div>
           ...
           <input type="number" name="offeredReturn" id="offeredReturn" required value={formData.offeredReturn} onChange={handleChange} className="input-field" placeholder="e.g., 10"/>
         </div>
         <div>
           <label htmlFor="dealLength" className="block text-sm font-medium text-primary">Est. Deal Length (Months)</label>
           <input type="number" name="dealLength" id="dealLength" required value={formData.dealLength} onChange={handleChange} className="input-field" placeholder="e.g., 12"/>
         </div>
         <div>
            ...
            <select id="lienPosition" name="lienPosition" required value={formData.lienPosition} onChange={handleChange} className="input-field">
                <option>1st</option>
                <option>2nd</option>
                <option>Other</option> {/* Added Other */} 
            </select>
        </div>
      </div>

      {/* Collateral and Funds Usage - Same as Gap */}
      <div className="grid grid-cols-1 md:grid-cols-1 gap-6">
        <div>
         ...
         <div>
            ...
            <textarea id="fundsUsage" name="fundsUsage" rows={4} required value={formData.fundsUsage} onChange={handleChange} className="input-field" placeholder="e.g., Acquisition: $200k, Rehab: $50k, Holding Costs: $10k..."></textarea>
        </div>
      </div>
